...
/**@brief Function for adding our new characterstic to "sensor service" that we initiated in the previous tutorial. 
 *
 * @param[in]   p_sensor_service        Sensor Service structure.
 *
 */
static uint32_t our_char_add(ble_os_t * p_sensor_service)
{
...
void sensor_service_init(ble_os_t * p_sensor_service)
{
    ...
    // OUR_JOB: Step 3.B, Set sensor service connection handle to default value. I.e. an invalid handle since we are not yet in a connection.
    p_sensor_service->conn_handle = BLE_CONN_HANDLE_INVALID;
